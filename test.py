...
model_path = './best_models/mcnn_shtechA_608.h5'
...
for blob in data_loader:
    ...
    if save_output:
        count_result = os.path.join(output_dir, 'result_count_' + blob['fname'].split('.')[0] + '_.txt')
        f = open(count_result, 'w')
        f.write('gt_count: %0.2f, et_count: %0.2f' % (gt_count, et_count))
        f.close()
